...
function UserPayResort() {
    ...
    const [bookdata,setBookdata] = useState({
        checkin: location.state.bookdata.checkin,
        checkout: location.state.bookdata.checkout,
        count: location.state.bookdata.count,
        children:location.state.bookdata.children,
        price: location.state.bookdata.price,
        resort_id:location.state.bookdata.resort_id,
        rooms:location.state.bookdata.rooms
    });
   ...
   useEffect(()=>{
    if(!payment){
        navigate("/userviewresort")
    }else{
       console.log(bookdata.rooms);
 total=bookdata.price*bookdata.rooms;
setTemp(total)
    }
   },[])
   const addContacts=(e)=>{
  ...
  console.log(params);
    fetch('http://localhost:5000/user/payResort', {
        method: 'POST',
